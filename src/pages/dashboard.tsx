...
import { getFaqs } from 'apiSdk/faqs';
...
function DashboardPage() {
  ...
  const faqStat = useSWR<PaginatedInterface<FaqInterface>>(
    () => `/faqs`,
    () => getFaqs({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Faqs',
      result: faqStat?.data?.totalCount,
      isLoading: faqStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
