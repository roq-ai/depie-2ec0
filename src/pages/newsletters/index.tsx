...
type ColumnType = ColumnDef<NewsletterInterface, unknown>;
...
export function NewsletterListPage(props: NewsletterListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<NewsletterInterface>>(
    () => `/newsletters?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: NewsletterInterface) => {
    if (hasAccess('newsletter', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/newsletters/view/${row.id}`);
    }
  };
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="newsletter" />
        </Flex>

        {hasAccess('newsletter', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/newsletters/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
