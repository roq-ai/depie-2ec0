...
import { getNewsletterById, updateNewsletterById } from 'apiSdk/newsletters';
...
function NewsletterEditPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<NewsletterInterface>(
    () => (id ? `/newsletters/${id}` : null),
    () => getNewsletterById(id),
  );
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: NewsletterInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateNewsletterById(id, values);
      mutate(updated);
      resetForm();
  ...
  const formik = useFormik<NewsletterInterface>({
    initialValues: data,
